...
const SESSION_KEY = 'try_rider_session';
...
interface AuthContextValue {
  ...
  verifyAndSignUp: (phoneNumber: string, code: string, fullName: string) => Promise<void>;
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  // a returning user straight to Home.
  ...
  const persistUser = async (account: RiderUser) => {
    setUser(account);
  ...
  const verifyAndSignUp = async (phoneNumber: string, code: string, fullName: string) => {
    setLoading(true);
    setError(null);
    try {
      const { verificationToken } = await postJson('/auth/otp/verify', { phoneNumber, code, purpose: 'rider_signup' });
      const account = await postJson('/auth/riders/signup', { phoneNumber, fullName, verificationToken });
  ...
  const verifyAndSignIn = async (phoneNumber: string, code: string) => {
    ...
    setError(null);
    try {
      const { verificationToken } = await postJson('/auth/otp/verify', { phoneNumber, code, purpose: 'rider_signin' });
      const account = await postJson('/auth/riders/signin', { phoneNumber, verificationToken });
      await persistUser(account);
    } catch (e: any) {
      setError(e.message ?? 'Something went wrong');
      throw e;
