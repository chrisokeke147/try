...
interface Params {
  phoneNumber: string;
  purpose: 'rider_signup' | 'rider_signin';
  fullName?: string; // only present for signup
}

export function OtpVerificationScreen({ navigation, route }: any) {
  const { phoneNumber, purpose, fullName } = route.params as Params;
  ...
  const verify = async () => {
    ...
    try {
      if (purpose === 'rider_signup') {
        await verifyAndSignUp(phoneNumber, code.trim(), fullName ?? '');
      } else {
